    ...
    return (
      <div className="fixed inset-0 z-10 flex items-center justify-center">
        ...
        <div className="z-20 bg-white p-8">
          <h2 className="mb-4 text-2xl font-bold">Selected Plan: {selectedPlan.name}</h2>
          <ul>
          ...
          <div className="flex justify-between mt-4">
            <button className="rounded bg-blue-500 px-4 py-2 text-white" onClick={onClose}>
              Close
            </button>
            <button
              className="rounded bg-red-500 px-4 py-2 text-white mr-2"
      ...
      <div className="flex flex-row justify-center mt-28 ml-44">
        {plans.map((plan) => (
          <div key={plan.name} className="m-2 w-1/3">
            <div className={`flex h-full flex-col justify-between rounded-lg ${plan.name === 'BASIC' ? 'bg-gradient-to-b from-red-700 to-red-300' : plan.name === 'PRIME' ? 'bg-gradient-to-b from-indigo-700 to-indigo-300' : 'bg-gradient-to-b from-purple-700 to-purple-300'} p-4 text-white shadow-lg transition-transform duration-300 ease-in-out hover:scale-105`}>
              <div>
                ...
                <ul className="list-inside list-none">
                  {plan.features.map((feature) => (
                    <li key={feature} className="mb-3 flex items-center font-semibold">
                      <svg
              ...
              onClick={() => handleSubscribe(plan)}
                className={`mt-4 self-center rounded ${plan.name === 'BASIC' ? 'bg-bronze' : plan.name === 'PRIME' ? 'bg-silver' : 'bg-gold'} px-4 bg-white py-2 text-black`}
              >
                SUBSCRIBE
